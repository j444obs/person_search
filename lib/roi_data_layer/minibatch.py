...
@Description: Compute minibatch blobs for training a Fast R-CNN network.
"""
...
def get_minibatch(roidb, num_classes):
    ...
    if cfg.TRAIN.HAS_RPN:
        ...
        if "gt_pids" in roidb[0]:
            gt_boxes = np.hstack([gt_boxes, roidb[0]["gt_pids"][gt_inds, np.newaxis]])
        ...
        for im_i in range(num_images):
            labels, _, im_rois, bbox_targets, bbox_inside_weights = _sample_rois(
...
def _vis_minibatch(im_blob, rois_blob, labels_blob, overlaps):
    ...
    for i in range(rois_blob.shape[0]):
        ...
        plt.gca().add_patch(plt.Rectangle((roi[0], roi[1]),
                                          roi[2] - roi[0],
                                          roi[3] - roi[1],
                                          fill=False,
                                          edgecolor="r",
...
def _vis_minibatch_(im_blob, gt_boxes):
    ...
    for x1, y1, x2, y2, is_person, pid in gt_boxes:
        ...
        if pid != -1:
            plt.gca().add_patch(plt.Rectangle((x1, y1 - 16), x2 - x1,
                                              16, fill=True, edgecolor=ec,
                                              linewidth=1, facecolor=ec))
